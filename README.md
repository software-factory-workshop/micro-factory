...
One v0 prototype in, one generated Nuxt 4 target (repository from the `adeo-nuxt-shell` template, git-linked Vercel project behind Passport, `main` deployed) and one verified pull request from `dev` to `main` with a live preview out, merged when a person approves in the cockpit. Built with Eve on the host mechanisms of the [Jira factory](https://github.com/software-factory-workshop/jira-clone) (`apps/factory`): the same durable delivery loop, receipt-first Blob ledger, station identity, Cedar authority and GitHub publication path, copied rather than redesigned.
...
| `software-factory-workshop/adeo-nuxt-shell` | Template. A Nuxt 4 shell on the ADEO layer with Vitest, headless e2e through `agent-browser`, and AWS DSQL dependencies. Every target repository (`adeo-<name>-nuxt`) is generated from it at delivery start; the migrator publishes on its `dev` branch. `adeo-todo-nuxt` is the 14 Sep reference target. |
...
| --- | --- | --- | --- |
| `migrator` | `prepare_work`, `bash`, `read_file`, `glob`, `grep`, `verify_work`, `publish_work`, `ask_owner`, `refresh_target` | `adeo-nuxt-ui`, `nuxt`, `vue`, `typescript-best-practices`, `vercel-connect`, `vercel-passport` | One PR from `dev` to `main`. `publish_work` refuses if the changed-file digest moved since `verify_work` or verification did not pass. |
