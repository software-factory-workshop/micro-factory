...
const brief = ref(`Migrate the v0 prototype ${prototypeRepository} into the Nuxt 4 shell in ${factoryRepository}. Every page and interaction of the prototype gets a Nuxt counterpart on the ADEO layer. Replace Neon with AWS DSQL behind one typed data module with the same query surface and a labelled fixture fallback. Keep Vercel Passport as the front door. The Jira issues section is live through Vercel Connect: the Jira clone MCP connector is user-authorised, so derive the Connect subject from the verified Passport identity, expose GET /api/jira/authorize (startAuthorization, scopes ['read'], callback to '/'), answer { source: 'consent_required', authorizeUrl } when consent is missing and show an 'Authorise with the Jira clone' button, otherwise call the MCP tool listIssues over Streamable HTTP with the bearer. Labelled fixtures remain only for the fixture-user identity without Passport and for persistence when no DSQL cluster is configured. Add Vitest unit tests and headless Playwright e2e tests through agent-browser.`);
